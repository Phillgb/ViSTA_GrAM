...
manual_initialisation = 'on' #'on' or 'off' - On means the veg and sand grids are manually specified in the 'startgrids_manual' function; off means grids are specified in the 'startgrids' functions
last_dir = '/home/phillgb/projects/def-kingj/phillgb/simulation1023_finalSed/' #Specify the directory where output files can be found to manually specify starting state of the next simulation
...
grass_proportion = 0.85 #Proportion of the veg distribution that is grass (must sum to 1 with shrub and tree)
shrub_proportion = 0.10 #Proportion of the veg distribution that is shrub (must sum to 1 with grass and tree)
...
wind_event_constant = 12.5 #For 'constant' (m/s) <--- Unobstructed windspeed entering grid for that timestep
