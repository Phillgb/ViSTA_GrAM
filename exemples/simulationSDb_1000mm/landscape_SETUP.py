...
start_grid_sand_upper = 1.5 #Highest depth of sediment when depths are randomly or uniformly assigned at start (m)
...
veg_distrib = 0.9 # % of surface covered in vegetation, min = 0, max = 1.0 (i.e. 0.9 means 90% of cells have veg)
grass_proportion = 0.8 #Proportion of the veg distribution that is grass (must sum to 1 with shrub and tree)
shrub_proportion = 0.1 #Proportion of the veg distribution that is shrub (must sum to 1 with grass and tree)
tree_proportion = 0.1 #Proportion of the veg distribution that is tree (must sum to 1 with grass and shrub)
...
wind_event_constant = 0.0 #For 'constant' (m/s) <--- Unobstructed windspeed entering grid for that timestep
...
rainfall_series_constant = 1000 #For constant stress on time series (mm, annual equivalent)
...
recolonisation_dynamism = 'off' #'off' means grass/tree/shrub proportions remain the same at every run; 'on' means plant recolonisation is dynamically dependent on current proportions of grass/shrub/tree in the domain
...
grazing_event_timeseries = 'none' #Type of grazing regime imposed: 'none', 'constant', 'periodic', 'GrAM'
...
grass_vmass = 0.4  #Volumetric mass equivalant for the grass in Kg/m3
...
wind_time = np.arange(1, wind_iterations+1)*(wind_event_frequency/veg_update_frequency*veg_update_freq_equivalent/12) #Equivalance of wind iterations in years
...
veg_time = np.arange(1, veg_iterations+1)*(veg_update_freq_equivalent/12) #Equivalance of veg iterations in years

# Grazing
grazing_iterations = math.floor(model_iterations/GrAM_frequency) #Number of times grazing event occurs using module GrAM, given the specifications above - DON'T CHANGE FORMULA
grazing_time = np.arange(1, grazing_iterations+1)*(GrAM_frequency/veg_update_frequency*veg_update_freq_equivalent/12) #Equivalance of grazing iterations on years
