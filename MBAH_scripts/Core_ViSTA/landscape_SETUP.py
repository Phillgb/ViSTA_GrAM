...
#Time
model_iterations = 500 #Total number of iterations model is running
wind_event_frequency = 1 #Frequency at which wind event occurs (i.e. 5 = one wind event every 5 model iterations)
wind_resolution = 12000  #Equivalent time for which wind is blowing during each wind event (MINUTES)
veg_update_frequency = 5 #Frequency at which veg is updated using the veg patterning module (i.e. 10 = one update every 10 model iterations)
...
moisture_update_frequency_equivalent = 200 #Only relevant for soil moisture feedback routine - it's what the moisture update frequency represents in real HOURS; technically should be compatible with veg update frequency and wind resolution
