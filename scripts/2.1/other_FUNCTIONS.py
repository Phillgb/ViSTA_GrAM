...
def startarrays():
    ...
    grazer_passage_grid = np.zeros((Nr, Nc)) # Grid to store the number of passage of each grazer on each cell (GrAM module).
    last_grazer_passage = np.zeros((Nr, Nc)) # Grid to store the previous non-wrap state of grazer passage after vegetation update (GrAM module).
    graz_update = np.zeros((Nr, Nc)) # Grid to store a update indices for grass cumulated growth updated after grazing event (GrAM module).
    ...
    return (rainfall_days, total_sand_vol, total_aval_vol, total_veg_pop, average_age_table, veg_proportions, exposed_wall_proportions, differences_grid, avalanching_grid, windspeed_grid,
     interaction_field, grazer_passage_grid, last_grazer_passage, graz_update, avail_forage, veg_growth_factor, actual_grazed_series, veg_growth_mass)
#*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
...
def startgrids_3(sand_heights_grid, veg_grid, veg_type_grid):    
    ...
    initial_apparent_veg_type_grid = copy.copy(veg_type_grid); initial_apparent_veg_type_grid[np.where(initial_veg_grid <= veg_threshold)] = 0 #It's APPARENT because all cells are attributed a potential veg type, but those that have zero height need to be seen as zero 
