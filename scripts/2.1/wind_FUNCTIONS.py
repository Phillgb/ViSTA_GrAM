...
def avalanche(w_sand_heights_grid, w_veg_grid, w_rooting_heights_grid, w_walls_grid):
    ...
    i_random = np.zeros((Nr*Nc), dtype=int); j_random = np.zeros((Nr*Nc), dtype=int)
    for a in range(Nr):
        for b in range(Nc):
            i_random[a*Nc + b] = a        
    for a in range(Nr):
...
def depositiontrajectories(wind_angle):
    ...
    for i in range(Nrww):
        for j in range(Ncww):
            ...
            checked_cells = np.zeros((Nrww*Ncww*3, 2), dtype=int) #Multiply by 3 because want one cell either side of the checked cell
            path_counter = -1       
...
def sand_movement(w_sand_heights_grid, w_veg_grid, w_rooting_heights_grid, w_soil_moisture_grid, w_porosity_grid, w_walls_grid, windspeed_grid, sed_balance_moisture_grid, path_cell_location, path_cell_location_depos, path_depos, prob_depos_bare, prob_depos_sand, rainfall_presence, soil_moisture_presence, t):
    ...
    for r in range(Nr): 
        for c in range(Nc):
            ...
            temp_path_cell_location = np.zeros(number_path_cells, dtype=int) #To fill with location of each downwind cell, in the reshaped array
    ...
    for q in range(Nr*Nc):
        ...
        exposed_veg_height = (w_rooting_reshaped[current_target_cell_index] + w_veg_reshaped[current_target_cell_index]) - w_sand_heights_reshaped[current_target_cell_index] # Vegetation height above ground allow to determine possibility of transport
        
        # ----------------------------- Erosion -------------------------------
        #Only erode if target cell is NOT IN SHADOW ZONE and NOT TOO MOIST and A WALL ISN'T EXPOSED
        if shadow_zones_array[current_target_cell_index] == 0 and eros_multiplier > 0 and exposed_wall_height <= 0 and exposed_veg_height > veg_effectiveness_threshold_prob_depos:      
            ...
            if cell_flux > 0:
                ...
                if dh > 0: #Only do deposition routine if sand has left the polled cell
                    ...
                    number_path_cells = np.size((np.where(path_depos[current_target_cell_index, :] > 0.))) #Pinpoints location of polled cell
                    temp_path_cell_location = np.zeros(n_shells, dtype=int) #To fill with location of each downwind cell, in the reshaped array
