...
for t in range(model_iterations):
    ...
    if grazing_event_timeseries == 'GrAM':
        if (t+1) % GrAM_frequency == 0:
            ...
            num_grazer = np.floor(stocking_rate * (Nc*Nr*np.power(cell_width, 2)/10000) * (grazing_time[1]*365/GrAM_event_duration)).astype(int) # grazing_time[1] correspond to 1 grazing iteration in terms of year
